...
void ShaderProgram::compile(renderdevice::IShader *pVertexShader, renderdevice::IShader *pPixelShader){
    ...
    for(int i = 0; i < numActiveUniforms; i++){
        ...
        if((paramType == renderdevice::kTypeTexture2D) || (paramType == renderdevice::kTypeTexture3D)){
            m_paramBlock.addShaderParam(renderdevice::opengl::ShaderParam(uniformName, paramType, 0, uniformHandle, textureUnitCounter++));
        }else{
            m_paramBlock.addShaderParam(renderdevice::opengl::ShaderParam(uniformName, paramType, 0, uniformHandle));
        }
    }
}
...
renderdevice::IShaderParamBlock* ShaderProgram::createShaderParamBlock(const char *names[], size_t numNames){
    ...
    if(names == 0 || numNames == 0){
        ...
        pBlock = new /*()*/ renderdevice::opengl::ShaderParamBlock(m_paramBlock);
    }else{
        // return block with the selected parameters
        pBlock = new /*()*/ renderdevice::opengl::ShaderParamBlock();
...
void ShaderProgram::setShaderParamBlock(renderdevice::IShaderParamBlock const &block){
    ...
    for(size_t i = 0; i < numParameters; i++){
        ...
        renderdevice::opengl::ShaderParam const *pGLParam = dynamic_cast<renderdevice::opengl::ShaderParam const*>(pParam);
...
//! Sets a param
void ShaderProgram::setGLUniformParam(renderdevice::opengl::ShaderParam const &param, void *pData){
