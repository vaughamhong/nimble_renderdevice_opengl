...
void RenderDevice::setGLRenderStateLighting(bool enabled){
	if(enabled){
		GLDEBUG(glEnable(GL_LIGHTING));
	}else{
		GLDEBUG(glDisable(GL_LIGHTING));
	}
}
...
void RenderDevice::setFillMode(renderdevice::eRenderFace face, renderdevice::eRenderFillMode mode){
    unsigned int glFaceEnum = 0;
	if(face == renderdevice::kRenderFaceFront){glFaceEnum |= GL_FRONT;}
	if(face == renderdevice::kRenderFaceBack){glFaceEnum |= GL_BACK;}
    if(face == renderdevice::kRenderFaceFrontAndBack){glFaceEnum |= GL_FRONT_AND_BACK;}
    
    unsigned int glFillModeEnum = 0;
	if(mode == renderdevice::kRenderFillModePoint){glFillModeEnum |= GL_POINT;}
	if(mode == renderdevice::kRenderFillModeLine){glFillModeEnum |= GL_LINE;}
    if(mode == renderdevice::kRenderFillModeFill){glFillModeEnum |= GL_FILL;}
    
	core::assert_error(glFaceEnum != 0);
	core::assert_error(glFillModeEnum != 0);
    GLDEBUG(glPolygonMode(glFaceEnum, glFillModeEnum));
}
//! Depth functions
